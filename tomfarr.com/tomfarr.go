/* Catalog parser for http://www.tomfarr.com/ */
package main
...
const ( // {{{
	baseURL    = "http://www.tomfarr.com"
	outputDir  = "images"
	outputJSON = "tomfarr.json"
) // }}}
...
func getCategories() map[string]string { // {{{
	...
	doc := getPageDoc(baseURL + "/collection/")
	doc.Find("div.catalog-filter-block > ul.catalog-filter-categories > li").Each(func(i int, s *goquery.Selection) {
...
func walkinPages() { // {{{
	...
	for cat, catLink := range categories {
		...
		for i := 1; i <= last; i++ {
			...
			go getProducts(getPageDoc(currPage), cat)
			wg.Add(1)
		}
	}
	wg.Wait()
...
func getProducts(doc *goquery.Document, cat string) { // {{{
	...
	doc.Find(`div.items > div.units-row > div.item`).Each(func(i int, s *goquery.Selection) {
		p := Product{}
		p.Category = cat

		tmpName, _ := s.Find("img.item-preview-image").Attr("alt")
		p.Name = tmpName
		...
		link, _ := s.Find("div.preview-picture > a.detail-page-link").Attr("href")
		// fmt.Printf("# Link - %s\n", link)
		p.Link = baseURL + link
