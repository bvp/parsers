...
const ( // {{{
	baseURL    = "http://www.emae.ru"
	outputDir  = "images"
	outputJSON = "emae.json"
) // }}}
...
func getCategories() map[string]string { // {{{
	...
	getPageDoc(baseURL + "/shop/").Find("div.top_box_wrap > div.top_box > div.menu_item").Each(func(i int, s *goquery.Selection) {
		cat, _ := s.Find("div > a.menu_link").Attr("title")
		//		catLink, _ := s.Find("div > a.menu_link").Attr("href")
		if !contains(excludeList, cat) {
			//			fmt.Printf("'%s' => ,\n", cat)
			//			fmt.Printf("'%s' => %s,\n", cat, baseURL+catLink)
			//			categories[cat] = baseURL + catLink
			s.Find("ul.cat_menu > li").Each(func(j int, ss *goquery.Selection) {
				tmpsCatID, _ := ss.Find("a.years_menu").Attr("id")
				sCatID := strings.TrimLeft(tmpsCatID, "menu_")
				sCat, _ := ss.Find("a.years_menu").Attr("title")
				//				sCatLink, _ := ss.Find("a.years_menu").Attr("href")
...
func walkinPages() { // {{{
	...
	for cat, catLink := range categories {
		...
		getProducts(getPageDoc(catLink+"&pageall=1"), cat)
		//		wg.Add(1)
		//		}
	}
	wg.Wait()
...
func getProducts(doc *goquery.Document, cat string) { // {{{
	...
	doc.Find(`div.catalog > div:nth-child(1) > div.item`).Each(func(i int, s *goquery.Selection) {
		p := Product{}
		p.Category = cat

		p.Name = s.Find("a:nth-child(1) > div.name").First().Text()
		tmpPict, _ := s.Find("a:nth-child(1) > div.image > img").Attr("src")
		p.Pict = baseURL + "/shop/" + tmpPict

		link, _ := s.Find("div.other_sizes_images > div.first_img_other > a").Attr("href")
		p.Link = baseURL + "/shop/" + link

		//		tID, _ := s.Find("div.first_img_other > div.first_img_other > a > img").Attr("data-tovar")
