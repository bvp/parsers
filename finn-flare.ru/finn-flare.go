/* Catalog parser for http://www.finn-flare.ru */
package main
...
const ( // {{{
	baseURL    = "http://www.finn-flare.ru"
	outputDir  = "images"
	outputJSON = "finn-flare.json"
) // }}}
...
func getCategories() map[string]string { // {{{
	...
	doc := getPageDoc(baseURL + "/catalog/")
	doc.Find("body > div.container > div > div.sidebar > div.h2").Each(func(i int, s *goquery.Selection) {
		cat := s.Text()
		//		catLink, _ := s.Find("div > a.menu_link").Attr("href")
		if !contains(excludeList, cat) {
			//			fmt.Printf("%d '%s' => ,\n", i, cat)
			//			fmt.Printf("'%s' => %s,\n", cat, baseURL+catLink)
			//			categories[cat] = baseURL + catLink
			doc.Find("body > div.container > div > div.sidebar > ul:nth-child(" + strconv.Itoa((i+1)*2) + ") > li").Each(func(j int, ss *goquery.Selection) {
				sCat := ss.Find("a").Text()
...
func getPages(url string) int { // {{{
	...
	doc.Find("body > div.container > div > div.contentMain > div.content.cf > div.catalog-list.cf > div.pager > div.modern-page-navigation > a").Each(func(i int, s *goquery.Selection) {
...
func walkinPages() { // {{{
	...
	for cat, catLink := range categories {
		...
		getProducts(getPageDoc(catLink+"?SHOWALL_1=1"), cat)
		//		wg.Add(1)
		//		}
	}
	wg.Wait()
...
func getProducts(doc *goquery.Document, cat string) { // {{{
	...
	doc.Find(`body > div.container > div > div.contentMain > div.content.cf > div.catalog-list.cf > div.catalog-items.cf > a.item`).Each(func(i int, s *goquery.Selection) {
		p := Product{}
		p.Category = cat

		p.Name = s.Find("span.descr > div.name").First().Text()
		p.Sku = strings.TrimLeft(s.Find("span.descr > div.article").First().Text(), "Артикул: ")
		...
		tmpPict, _ := docp.Find("#element_zoom > div.item.active > a > img").Attr("src")
