/* Catalog parser for http://www.etti-detti.net/ */
package main
...
const ( // {{{
	baseURL    = "http://www.etti-detti.net"
	outputDir  = "images"
	outputJSON = "etti-detti.json"
) // }}}
...
func getCategories() map[string]string { // {{{
	...
	doc.Find("#container_left_sidebar > div.cpt_category_tree > ul > li.parent").Each(func(i int, s *goquery.Selection) {
		cat := strings.TrimRight(strings.TrimSpace(s.Find("a").Text()), ".")
		...
		if !contains(excludeList, cat) {
			...
			sdoc.Find("#container_left_sidebar > div.cpt_category_tree > ul > li.child").Each(func(j int, ss *goquery.Selection) {
				sCat := strings.TrimLeft(ss.Find("a").Text(), "- ")
...
func getPages(url string) int { // {{{
	...
	doc.Find("#container_main_content > div.cpt_maincontent > center > div.b-pagenavi > a").Each(func(i int, s *goquery.Selection) {
		if !contains(excludeList, s.Text()) {
			fmt.Printf("%d - %s\n", i, s.Text())
			ta = append(ta, s.Text())
		}
	})
	// body > div.container > div > div.contentMain > div.content.cf > div.catalog-list.cf > div.pager > div > a:nth-child(8)
...
func walkinPages() { // {{{
	...
	for cat, catLink := range categories {
		...
		getProducts(getPageDoc(catLink+"all/"), cat)
		//		wg.Add(1)
		//		}
	}
	wg.Wait()
...
func getProducts(doc *goquery.Document, cat string) { // {{{
	...
	//	ret, _ := doc.Find(`table.product_brief_t`).Html()
	//	fmt.Printf("HTML - %s\n", ret)
	doc.Find(`table.product_brief_t`).Each(func(i int, s *goquery.Selection) {
		p := Product{}
		p.Category = cat

		p.Name = s.Find("div.prdbrief_name > a").First().Text()
		p.Sku = p.Name
		link, _ := s.Find("div.prdbrief_name > a").First().Attr("href")
		p.Link = baseURL + link

		docp := getPageDoc(p.Link)

		tmpPict, _ := docp.Find("img#img-current_picture").Attr("src")
		...
		docp.Find("div.cpt_product_description > div > p").Each(func(i int, sd *goquery.Selection) {
			content := strings.TrimSpace(sd.Text())
			//			content := strings.TrimLeft(sd.Text(), title[0]+":")
			if content != "" {
		...
		if p.Name == "" {
			failed = append(failed, "SKU: "+p.Sku+", Name: "+p.Name+", Category: "+p.Category+", Link: "+p.Link)
			doc.Next()
		}
