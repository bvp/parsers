/* Catalog parser for http://obuv-top-top.ru */
package main
...
const ( // {{{
	baseURL    = "http://obuv-top-top.ru"
	outputDir  = "images"
	outputJSON = "obuv-top-top.json"
) // }}}
...
func getCategories() map[string]string { // {{{
	...
	getPageDoc(baseURL + "/shop/catalog").Find("#yw2_tab_0 > form > div:nth-child(3) > div:nth-child(2) > div > div.choice").Each(func(i int, s *goquery.Selection) {
		cat := s.Find("label > a").Text()
		catLink, _ := s.Find("label > a").First().Attr("href")
...
func getProducts(doc *goquery.Document, cat string) { // {{{
	...
	doc.Find(`#yw0 > div.items > div.product_div`).Each(func(i int, s *goquery.Selection) {
		link, _ := s.Find("div.product_catalog_wrapper > a.product__name").First().Attr("href")
...
func getProductInfo(url string, cat string) { // {{{
	...
	//	p.Sku = tmpSKU
	p.Name = strings.TrimLeft(strings.TrimRight(strings.TrimLeft(docp.Find("head > title").Text(), tmpSKU+" "), "артикул: "+tmpSKU), ".")
