/* Catalog parser for http://www.karapuzik37.ru */
package main
...
const ( // {{{
	baseURL    = "http://www.karapuzik37.ru"
	outputDir  = "images"
	outputJSON = "karapuzik37.json"
) // }}}
...
func getCategories() map[string]string { // {{{
	...
	doc.Find("#sidebar > div.module").Each(func(i int, s *goquery.Selection) {
		cat := strings.TrimSpace(s.Find("div.moduletitle").Text())
		//		catLink, _ := s.Find("div > a.menu_link").Attr("href")
		if !contains(excludeList, cat) {
			//			fmt.Printf("%d '%s' => ,\n", i, cat)
			//			fmt.Printf("'%s' => %s,\n", cat, baseURL+catLink)
			//			categories[cat] = baseURL + catLink
			s.Find("div.modulebody > div > ul.inmag_tree > li").Each(func(j int, ss *goquery.Selection) {
				sCat := strings.TrimSpace(strings.Replace(ss.Find("a").Text(), "\n", "", -1))
...
func getProducts(doc *goquery.Document, cat string) { // {{{
	...
	doc.Find(`table.mod_inmag_front > tbody > tr > td`).Each(func(i int, s *goquery.Selection) {
		p := Product{}
		p.Category = cat

		p.Name = s.Find("div.item_wrap > table > tbody > tr > td > div:nth-child(1)").First().Text()
		p.Sku = strings.TrimLeft(s.Find("div:nth-child(3) > div:nth-child(1)").First().Text(), "Арт ")

		// link, _ := s.Attr("href")
		// p.Link = baseURL + link
		p.Link = categories[cat]

		//		docp := getPageDoc(p.Link)

		tmpPict, _ := s.Find("a.highslide-image").Attr("href")
